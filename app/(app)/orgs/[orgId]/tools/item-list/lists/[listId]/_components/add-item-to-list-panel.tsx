...
export function AddItemToListPanel({
  ...
  const position =
    !isNaN(parsedPage) && !isNaN(parsedCol) && !isNaN(parsedRow)
      ...
      : -1;
  const canAdd = selectedItem !== null && position >= 0;

  function handleAdd() {
    if (!selectedItem || position < 0 || isPending) return;
      ...
      <div className="px-4 py-3 border-t border-border flex flex-col gap-3">
        {selectedItem ? (
          ...
          <p className="text-xs text-muted-foreground">
            Select an item above or drag it to a cell
          </p>
        )}
        ...
        <Button onClick={handleAdd} disabled={!canAdd || isPending} className="w-full">
          Add to list
        </Button>
      </div>
    </div>
